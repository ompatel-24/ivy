...
// bitsets. Prime that hook as disabled only when Rome first needs a QR code,
...
func disableDependencyDebug() {
	disableDependencyDebugOnce.Do(func() {
		...
		_, _ = qrcode.NewWith("rome", qrcode.WithErrorCorrectionLevel(qrcode.ErrorCorrectionMedium))
		if present {
