...
Rome is an open-source tool for running any interactive CLI inside a
...
Rome is packaged as versioned, verifiable release archives and a Homebrew
formula. The mobile-first xterm.js client remains embedded inside the Rome
binary.
Setting `ROME_LISTEN` adds:
...
Version 0.2.0 is the first Rome-branded release. It intentionally renames the
binary, Go module, environment variables, browser storage keys, and WebSocket
subprotocols together. Existing Ivy 0.1.0 installations do not upgrade in
place; remove the old formula before installing Rome:
...
archive contains only `rome`, `README.md`, and `LICENSE`. To install an archive,
download the one for your platform, verify it, and place `rome` somewhere on
your `PATH`:

```bash
shasum -a 256 -c rome_0.2.0_checksums.txt
gh attestation verify rome_0.2.0_darwin_arm64.tar.gz --repo ompatel-24/rome
tar -xzf rome_0.2.0_darwin_arm64.tar.gz
...
artifact was built by Rome's release workflow.

## Build

Rome requires Go 1.23 or newer. `make build` also requires a Vite-compatible
...
is never needed to run Rome.

```bash
make build          # produces the self-contained dist/rome
./dist/rome bash
```
...
Port `0` requests an available ephemeral port. Rome rejects wildcard listeners
...
When enabled in an interactive terminal, Rome prints a QR code plus a URL such
as `http://127.0.0.1:7654/s/<id>#token=<token>`. Scan the QR code, open the URL
directly, or bind Rome to your Mac's concrete LAN address and scan it from a
...
Rome's future hosted relay will address that reachability problem.

Mobile assets are embedded in every Go build. `ROME_WEB_DIR` remains an explicit
development/testing override; when set, Rome requires that directory to contain
...
Each subscriber has a 64-chunk queue. Rome disconnects a subscriber that cannot
...
Rome publishes macOS and Linux archives for amd64 and arm64. The browser client
